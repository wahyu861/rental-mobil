<section class="blog-detail-banner-2 py-40">
    <div class="container-fluid">
        <div class="row ">
            <div class="col-lg-8">
                <p class="mt-48 mb-24">{!! $blog->content !!}</p>
            </div>
            <div class="col-lg-4">
                <div class="sides-bar mt-24">
                    ...
                    @foreach ($relatedBlogs as $relatedBlog)
                        <div class="pic mb-24">
                            <a href="{{ url('blogs/' . $relatedBlog->slug) }}">
                                <img src="{{ asset('storage/' . $relatedBlog->image_cover) }}"
                                    alt="{{ $relatedBlog->title }}">
                            </a>
                        </div>
                        <div class="text-block">
                            <p class="mb-12">{{ $relatedBlog->created_at->format('F j, Y') }}</p>
                            <a href="{{ url('blogs/' . $relatedBlog->slug) }}"
                                class="title">{{ $relatedBlog->title }}</a>
                            <p class="mb-24">{!! \Illuminate\Support\Str::limit(strip_tags($relatedBlog->content), 100) !!}</p>
                            <a href="{{ url('blogs/' . $relatedBlog->slug) }}" class="cus-btn-2">
                                <span class="btn-text">
                                    See Details
